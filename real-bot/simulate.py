...
class SimulationSession(BettingSession):
    ...
    def resolve_all_bets(self, minutes=1, use_real_price=True):
        """Resolve all open bets - optionally fetch real price"""
        resolved = []
        ...
        for bet in self.open_bets:
            ...
            if use_real_price and market_id:
                exit_price = fetch_market_price(market_id)
                if exit_price is None:
                    print(f"⚠️ Could not fetch real price for {market_id[:20]}..., using simulated")
    ...
    def fill_open_bets():
        ...
        while get_open_amount() < target_amount and high_prob:
            ...
            bet = {
                ...
                'action': 'NO',
                'entry_price': market.get('entry_price', 0.5),
                'entry_prob': 1 - market['prob_yes'],
                'amount': current_bet_size,
                'odds': 1/(1-market['prob_yes']) if market['prob_yes'] < 1 else 1,
                'opened_at': datetime.now(),
                'category': market.get('category', 'Other'),
                'url': market.get('url', '')
            }
